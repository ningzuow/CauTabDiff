...
class UnifiedCtimeDiffusion(torch.nn.Module):
    def __init__(
        ...
        idx_name_mapping = kwargs.get("idx_name_mapping", None)
        num_col_idx = kwargs.get("num_col_idx", None)
        cat_col_idx = kwargs.get("cat_col_idx", None)
        target_col_idx = kwargs.get("target_col_idx", None)
        causal_layers_path = kwargs.get("causal_layers_path", None)

        # 默认：所有列 scale = 1
        self.num_layer_scale = torch.ones(self.num_numerical_features, dtype=torch.float32)
        self.cat_layer_scale = torch.ones(len(self.num_classes), dtype=torch.float32)

        if (
            idx_name_mapping is not None
            and num_col_idx is not None
            and cat_col_idx is not None
            and causal_layers_path is not None
            and os.path.exists(causal_layers_path)
        ):
            # JSON 的 key 是 str，转 int
            idx_name_mapping = {int(k): v for k, v in idx_name_mapping.items()}

            with open(causal_layers_path, "r") as f:
                layer_cfg = json.load(f)

            num_layers = layer_cfg["num_layers"]
            layer_scales = torch.linspace(0.6, 1.2, steps=num_layers)

            def get_scale_for_feat(feat_name: str) -> float:
                for layer_id in range(num_layers):
                    layer_key = f"layer{layer_id}"
                    if feat_name in layer_cfg.get(layer_key, []):
                        return layer_scales[layer_id].item()
                # 不在任意层配置里，就保持 1.0
                return 1.0

            # ----- 数值列：local_idx -> global_idx -> feat_name -----
            for local_i, global_idx in enumerate(num_col_idx):
                feat_name = idx_name_mapping.get(global_idx, None)
                if feat_name is None:
                    continue
                self.num_layer_scale[local_i] = get_scale_for_feat(feat_name)

            # ----- 类别列（不含 label）：前 len(cat_col_idx) 个 -----
            for local_i, global_idx in enumerate(cat_col_idx):
                feat_name = idx_name_mapping.get(global_idx, None)
                if feat_name is None:
                    continue
                self.cat_layer_scale[local_i] = get_scale_for_feat(feat_name)

            # ----- 最后一个 categorical 位置通常是 label（如 income）-----
            if (
                target_col_idx is not None
                and len(target_col_idx) > 0
                and len(self.num_classes) > len(cat_col_idx)
            ):
                label_global_idx = target_col_idx[0]
                feat_name = idx_name_mapping.get(label_global_idx, None)
                if feat_name is not None:
                    self.cat_layer_scale[len(self.num_classes) - 1] = get_scale_for_feat(feat_name)

        # 打印检查
        print("Final per-category noise scale:", self.cat_layer_scale)
        print("Final per-feature noise scale:", self.num_layer_scale)
    ...
    def mixed_loss(self, x):
        ...
        # === (新增) 按因果层级调整类别噪声 ===
        sigma_cat_scaled = sigma_cat * self.cat_layer_scale.to(sigma_cat.device)
        alpha = torch.exp(-sigma_cat_scaled)
        move_chance = -torch.expm1(-sigma_cat_scaled)      # torch.expm1 gives better numertical stability
        ...
        if x_num.shape[1] > 0:
            ...
            # 保证 shape 正确：[1,d_num] 才能和 [bs,d_num] 广播
            layer_scale = self.num_layer_scale.to(sigma_num.device).view(1, -1)
            sigma_num_scaled = sigma_num * layer_scale

            x_num_t = x_num + noise * sigma_num_scaled
