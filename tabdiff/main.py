...
def main(args):
    ...
    # === 新增：把列索引 -> 特征名映射、layers.json 路径传给扩散模型 ===
    raw_config['diffusion_params']['idx_name_mapping'] = info['idx_name_mapping']
    raw_config['diffusion_params']['causal_layers_path'] = os.path.join(data_dir, "layers.json")

    
    for k in ["idx_name_mapping", "num_col_idx", "cat_col_idx", "target_col_idx", "causal_layers_path"]:
        if k in raw_config["diffusion_params"]:
            del raw_config["diffusion_params"][k]
    ...
    diffusion = UnifiedCtimeDiffusion(
        ...
        **raw_config['diffusion_params'],
        ...
        causal_layers_path=f"data/{dataname}/layers.json",
    )
